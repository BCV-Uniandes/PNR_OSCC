...
def keyframe_distance(_preds, _labels, samp_rates,full_gt = None):
    _preds_indv = np.argmax(_preds,axis=1)
    if full_gt is None:
        frame_labels=np.round(samp_rates*_labels)
    else:
        frame_labels = full_gt
    frame_preds=np.round(samp_rates*_preds_indv)
    err_frame = abs(frame_labels-frame_preds)
    fps=30
