...
class SampleFrames:
    ...
    def get_seq_frames_samp(self, num_frames):

        seg_size = float(num_frames - 1) / self.clip_len
        seq = []
        start = int(np.round(seg_size * 0))
        end = int(np.round(seg_size *  1))
        if not self.test_mode:
        ...
        for i in range(self.clip_len):
                seq.append(first)
                first = int(np.round(first+seg_size))
        return np.array(seq)
    ...
    def __call__(self, results):
        ...
        #CE: I CHANGED THIS CODE TO ADAPT FOR PNR FRAME SAMPLING ALWAYS
        if self.frame_uniform:  # sthv2 sampling strategy
            results['start_index'] = 0
            frame_inds = self.get_seq_frames(total_frames)
            pnr_frame=results['label']-results['offset']
            results['samp_rate']=results['total_frames']/self.clip_len
            #if len(np.argwhere(frame_inds<=pnr_frame))==0:
            #    pnr_ind=0
            #else:
            #    pnr_ind=np.argwhere(frame_inds<=pnr_frame)[-1][0]

            pnr_ind = int(np.round(pnr_frame/results['samp_rate']))#int(np.argmin(abs(frame_inds-pnr_frame)))
            pnr_ind = min(pnr_ind,self.clip_len-1)
            frame_inds[pnr_ind]=pnr_frame
            results['label']=int(pnr_ind)
            #breakpoint()
            #results['samp_rate']=pnr_frame/(pnr_ind+1)
        elif self.frame_test:
            results['start_index'] = 0
            frame_inds = self.get_seq_frames(total_frames)
            pnr_frame=0#results['label']-results['offset']
            results['samp_rate']=results['total_frames']/self.clip_len
            pnr_ind = int(np.round(pnr_frame/results['samp_rate']))#int(np.argmin(abs(frame_inds-pnr_frame)))
            pnr_ind = min(pnr_ind,self.clip_len-1)
            #frame_inds[pnr_ind]=pnr_frame
