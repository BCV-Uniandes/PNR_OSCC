...
from ..core import (mean_average_precision, mean_class_accuracy,
                    mmit_mean_average_precision, top_k_accuracy,keyframe_distance)
...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def label2array(num, label):
        ...
        arr[label] = 1.
        return arr
    def convert_data(self,gt_labels,results):
        preds=[res[0] for i,res in enumerate(results) if i%2==0 ]
        samp_rate=[res[0] for i,res in enumerate(results) if i%2!=0 ]
        converted_gt = np.nan_to_num(np.array(gt_labels)/np.array(samp_rate)).astype(int)
        return preds,samp_rate,converted_gt
    ...
    def evaluate(self,
        ...
        allowed_metrics = [
            ...
            'mmit_mean_average_precision','keyframe_distance',
        ]
        ...
        gt_labels = [ann['label']- ann['offset'] for ann in self.video_infos]
        preds,samp_rate,converted_gt = self.convert_data(gt_labels,results)
        ...
        for metric in metrics:
            ...
            if metric == 'keyframe_distance':
                distance = keyframe_distance(preds,None,samp_rate,gt_labels)
                eval_results['keyframe_distance'] = distance
                log_msg = f'keyframe_distance\t{distance:.4f}'
            ...
            if metric == 'top_k_accuracy':
                ...
                top_k_acc = top_k_accuracy(preds, converted_gt, topk)
                log_msg = []
            ...
            if metric == 'mean_class_accuracy':
                mean_acc = mean_class_accuracy(preds, converted_gt)
